...
func (parser *InternalGrammarParser) parseFunction(tokens []Token, startIndex int) (mentalese.Rule, int, bool) {
	...
	if ok {
		_, newStartIndex, ok = parser.parseSingleToken(tokens, startIndex, tMaps)
		if ok {
			startIndex = newStartIndex
			variables, startIndex, ok = parser.parseVariables(tokens, startIndex)
		} else {
			ok = true
		}
		if ok {
...
func (parser *InternalGrammarParser) parseResultHandler(tokens []Token, startIndex int) (mentalese.ResultHandler, int, bool) {
	...
	callback := func(tokens []Token, startIndex int, key string) (int, bool, bool) {
		...
		case field_answer:
			...
			answerFound = true
		case field_result:
			resultHandler.Result, startIndex, ok = parser.parseTerm(tokens, startIndex, false)
...
func (parser *InternalGrammarParser) parseVariableStructureRelation(tokens []Token, startIndex int) (mentalese.Term, int, bool) {
	...
	if ok {
		...
		if ok {
			indexTerm, newStartIndex, ok = parser.parseTerm(tokens, newStartIndex, false)
			if ok {
...
func (parser *InternalGrammarParser) parseKeywordRelation(tokens []Token, startIndex int, useAlias bool) (mentalese.Relation, int, bool) {
	...
	if ok1 {
		...
		if ok1 {
			startIndex = newStartIndex
			term1, startIndex, ok2 = parser.parseTerm(tokens, startIndex, false)
			...
			term2, startIndex, ok4 = parser.parseTerm(tokens, startIndex, false)
...
func (parser *InternalGrammarParser) parseAssignment(tokens []Token, startIndex int) (mentalese.Relation, int, bool) {
	ok := false
	relation := mentalese.Relation{}
	...
	if ok && len(variables) > 0 {
		...
		if ok {
			...
			if ok {
				leftHand := mentalese.Term{}
				if len(variables) == 1 {
					leftHand = mentalese.NewTermVariable(variables[0])
				} else {
					variableTerms := []mentalese.Term{}
					for _, v := range variables {
						variableTerms = append(variableTerms, mentalese.NewTermVariable(v))
					}
					leftHand = mentalese.NewTermList(variableTerms)
				}
			...
			for ok {
				if len(arguments) > 0 {
					...
					if !commaFound {
						...
						argument, startIndex, ok = parser.parseTerm(tokens, startIndex, false)
						if ok {
					...
					argument, newStartIndex, argumentFound = parser.parseTerm(tokens, startIndex, false)
					if !argumentFound {
						break
					} else {
						arguments = append(arguments, argument)
...
func (parser *InternalGrammarParser) parseBinding(tokens []Token, startIndex int) (mentalese.Binding, int, bool) {
	...
	for ok {
		...
		if ok {
			_, startIndex, ok = parser.parseSingleToken(tokens, startIndex, tColon)
			if ok {
				value, startIndex, ok = parser.parseTerm(tokens, startIndex, false)
				if ok {
...
func (parser *InternalGrammarParser) parseTermList(tokens []Token, startIndex int) (mentalese.TermList, int, bool) {
	...
	_, startIndex, ok = parser.parseSingleToken(tokens, startIndex, tOpeningBracket)
	if ok {
		term, newStartIndex, ok = parser.parseTerm(tokens, startIndex, false)
		if ok {
...
func (parser *InternalGrammarParser) parseVariables(tokens []Token, startIndex int) ([]string, int, bool) {
	variables := []string{}
	var newStartIndex int
	ok := false
	variable := ""
	...
	return variables, startIndex, ok
}

func (parser *InternalGrammarParser) parseTerm(tokens []Token, startIndex int, singleRelation bool) (mentalese.Term, int, bool) {
	...
	// ( term )
	{
		term, newStartIndex, ok = parser.parseGroupTerm(tokens, startIndex)
		if ok {
			startIndex = newStartIndex
			goto end
		}
	...
	// { relations }
	{
		...
		relationSet, newStartIndex, ok = parser.parseBody(tokens, startIndex)
		if ok {
	...
	// goal(x) :- subgoals(x)
	// goals(X) is a rule, but is not needed as a term
	{
		rule := mentalese.Rule{}
...
func (parser *InternalGrammarParser) parseGroupTerm(tokens []Token, startIndex int) (mentalese.Term, int, bool) {
	newStartIndex := 0
	ok := false
	var term mentalese.Term

	_, newStartIndex, ok = parser.parseSingleToken(tokens, startIndex, tOpeningParenthesis)
	if ok {
		startIndex = newStartIndex
		term, newStartIndex, ok = parser.parseTerm(tokens, newStartIndex, false)
		if ok {
			startIndex = newStartIndex
			_, newStartIndex, ok = parser.parseSingleToken(tokens, startIndex, tClosingParenthesis)
			if ok {
				startIndex = newStartIndex
			}
		}
