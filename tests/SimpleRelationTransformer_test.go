...
//	transformedSet := transformer.Extract(relationSet.GetRelations())

	//if transformedSet != "[task(S1, list_customers)]" {
	//	test.Error(fmt.Printf("Error in result: %s", transformedSet))
	//}
	//
	//transformedSet = transformer2.Extract(relationSet.GetRelations())
	//
	//if transformedSet != "[task(S1, all) subject(E1) done()]" {
	//	test.Error(fmt.Printf("Error in result: %s", transformedSet))
	//}

	// replace

	//transformedSet = transformer2.Replace(relationSet.GetRelations())
	//
